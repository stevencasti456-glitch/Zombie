...
func update_health_bar(current: int, maximum: int) -> void:
	...
	if health_percent > 0.6:
		...
		fill_style.bg_color = Color(0.9, 0.9, 0.2)   # Amarillo
	else:
		fill_style.bg_color = Color(0.9, 0.2, 0.2)   # Rojo
...
func _process(_delta: float) -> void:
	# Actualizar vida del jugador cada frame
